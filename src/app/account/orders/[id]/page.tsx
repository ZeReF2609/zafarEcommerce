    ...
    <div>
        <div className='mb-6'>
            <Button asChild variant="outline">
                <Link href="/account/orders"> &larr; Volver a Pedidos</Link>
            </Button>
        </div>
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                <CardTitle className="text-2xl font-headline">Pedido #{order.id}</CardTitle>
                <p className="text-muted-foreground">Realizado el {order.date}</p>
            </div>
            <Badge
                variant={order.status === 'Entregado' ? 'default' : order.status === 'Cancelado' ? 'destructive' : 'secondary'}
                className={cn('mt-2 sm:mt-0', order.status === 'Entregado' && 'bg-green-600 text-white')}
            >
                {order.status}
            </Badge>
          </div>
        ...
        <CardContent>
          <div className="grid gap-8 md:grid-cols-3">
            <div className="md:col-span-2">
              <h3 className="font-semibold text-lg mb-4">Artículos en este pedido</h3>
